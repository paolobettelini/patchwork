...
operations, including the optional game-identity authority.
...
[game-auth]
process-session-hours = 8
```

- `server.address` and `server.port` select the bind socket. They default to
  `0.0.0.0:8080` if omitted.
- `server.base-path` mounts the entire service below one URL prefix and defaults
  to `/`. For example, `/patchwork` moves the SPA, static assets, account APIs,
  OAuth, GitHub, game-auth, and registry routes below `/patchwork/...`.
...
- `server.frontend-url` is the public browser URL used for redirects back to the web
  profile. It can differ from the bind address; its path is normalized to the
  effective base path.
...
- `email.FROM_ADDRESS` is the verified sender identity passed to Resend, including
...
- `game-auth.process-session-hours` controls in-memory game process identity
  lifetime and must be between 1 and 168 hours.
- game servers are not listed in this file. Any participating server registers
  at `/server/instances`, keeps the returned secret in RAM, and renews its
  10-minute lease with heartbeat requests.

`--address`, `--port`, and `--base-path` override the TOML values. The base path
must start with `/`; trailing slashes are normalized away. `--site-root`, or
...
serialized into the Leptos frontend or its WebAssembly bundle. Dynamic game
server secrets are generated by the backend and must remain in server RAM.
Production `email.FROM_ADDRESS` must use a Resend-verified sending domain.

In the GitHub App settings, configure the effective `github.callback_url` as
the Callback URL, set the Setup URL to
`http://localhost:8080/github/installation-complete` (using the production
origin and configured base path in production), and turn **Request user
authorization during installation** off. With `base-path = "/patchwork"`, for
example, those paths become `/patchwork/github/callback` and
`/patchwork/github/installation-complete`. The server automatically rebases the
configured callback URL onto the effective base path, including when the CLI
override is used; the GitHub App setting must match that resulting URL. User
linking already has its own state-protected flow.
...
handled client-side. Every path in this chapter is relative to `server.base-path`;
at `/patchwork`, for example, the JavaScript entry point is
`/patchwork/pkg/patchwork_web.js`. Requests outside that prefix return 404.
